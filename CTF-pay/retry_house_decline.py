#!/usr/bin/env python3
"""Repeatedly retry home broadband links, prioritizing hitting "direct final state card decline".

Purpose:
- The target is not to perform challenge, but to prioritize triggering:
  3DS2/authenticate -> setup_intent requires_payment_method -> card_declined/generic_decline
- Once Stripe challenge is detected, this script will immediately terminate the current round and retry the next round
...
def _prepare_attempt_config(config: Path, attempt: int) -> Path:
    """Generate temporary configuration by attempt to avoid time_on_page being fixed to the same value every round."""
